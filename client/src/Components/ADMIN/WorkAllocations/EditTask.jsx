...
const EditTask = () => {
  ...
  useEffect(() => {
    const fetchTask = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/auth/get_task/${taskId}`);  // API to fetch task by ID
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:3000/auth/edit_task/${taskId}`, task);  // API to update task
