...
const TeamTaskStatusChart = () => {
  const [date, setDate] = useState("");
  const [statusData, setStatusData] = useState(null);

  const fetchStatus = async (selectedDate) => {
    try {
      const res = await axios.get(`http://localhost:3000/admin/teamTask_status_by_date?date=${selectedDate}`);
      if (res.data.status) {
  ...
  return (
    <div className="bg-white p-4 rounded-xl shadow-md mt-4">
      <div className="mb-4">
        <label className="font-semibold text-gray-700">Team Tasks:</label>
        <input
          type="date"
          value={date}
...
export default TeamTaskStatusChart;
