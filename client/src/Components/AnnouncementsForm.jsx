...
const AnnouncementForm = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:3000/auth/get_teams")
      .then((res) => setTeams(res.data.Result));
  }, []);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (formData.category !== "all" && !formData.target) {
      alert("Please select a target");
      return;
    }
    axios
      .post("http://localhost:3000/auth/add_announcement", formData)
  ...
  return (
    ...
    >
      ...
      <select
        ...
        onChange={handleChange}
        disabled={formData.category === "all"}
      >
